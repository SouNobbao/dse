...
#if defined(_WIN64)
#define PEFILE_ORDINAL_FLAG IMAGE_ORDINAL_FLAG64
#define PEFILE_ORDINAL(x) IMAGE_ORDINAL64(x)
#else
#define PEFILE_ORDINAL_FLAG IMAGE_ORDINAL_FLAG32
#define PEFILE_ORDINAL(x) IMAGE_ORDINAL32(x)
#endif
//==============================================================================
...
bool PEFile::readImportTable() {
	...
	while (true) {
		...
		while (true) {
			if ((importThunk->u1.Ordinal & PEFILE_ORDINAL_FLAG) == PEFILE_ORDINAL_FLAG) {
				importFunction->FunctionId = PEFILE_ORDINAL(importThunk->u1.Ordinal);
			} else {
	...
	while (importDll != NULL) {
		...
		while (importFunction != NULL) {
			...
			if (importFunction->FunctionId != 0) {
				importThunk.u1.Ordinal =
					static_cast<decltype(importThunk.u1.Ordinal)>(importFunction->FunctionId) |
					static_cast<decltype(importThunk.u1.Ordinal)>(PEFILE_ORDINAL_FLAG);
			} else {
...
void PEFile::fixHeaders() {
	peHeaders.OptionalHeader.SizeOfHeaders = alignNumber(dosHeader.PEHeaderOffset + peHeaders.FileHeader.SizeOfOptionalHeader +
															 peHeaders.FileHeader.NumberOfSections * sizeof(PE_SECTION_HEADER),
														 peHeaders.OptionalHeader.FileAlignment);
